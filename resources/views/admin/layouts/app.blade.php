...
<head>
    ...
    <title>@yield('title', 'Admin Panel') — Luxe Furniture</title>
    ...
    <div class="flex h-screen overflow-hidden">
            ...
            <div class="p-4 border-t border-white/10">
                <div class="flex items-center gap-3 p-3 rounded-xl bg-white/5 mb-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <p class="text-white text-sm font-semibold truncate">{{ Auth::guard('admin')->user()->name }}</p>
                        <p class="text-warm-500 text-xs truncate">{{ Auth::guard('admin')->user()->email }}</p>
                    </div>
                </div>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white border-b border-warm-200 px-8 py-4 flex items-center justify-between shadow-sm shrink-0">
                <div>
                    <h1 class="text-xl font-serif font-bold text-warm-900">@yield('page_title', 'Dashboard')</h1>
                    <p class="text-warm-500 text-sm">@yield('page_subtitle', 'Selamat datang kembali!')</p>
                </div>
                <div class="flex items-center gap-3">
                    @if(session('success'))
                    ...
                    @endif
                    <span class="text-warm-400 text-sm">{{ now()->isoFormat('dddd, D MMMM Y') }}</span>
                </div>
            </header>
